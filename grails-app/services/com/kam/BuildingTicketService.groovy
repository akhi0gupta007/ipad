...
class BuildingTicketService {
    ...
    def getBuildingTickets(def object){
        ...
        log.debug "xml in Building Ticket Service is"+object
        ...
        log.debug "building received is =="+xmlBuilding
        ...
        if(dbUser){
            if(dbUser.enabled==true) {
                ...
                def building1 = Building.get(Long.parseLong(xmlBuilding.toString()))
                ...
                def report = getLatestReport(dbUser.username.toString(),building1)
                ...
                xml.'?xml version="1.0" encoding="UTF-8"?'{
                    ...
                    building{
                        ...
                        if(report && report instanceof Report)
                            checkBuilding = checkBuildings(report,building1)
                        else
                            checkBuilding = false
                        ...
                        if(report && report instanceof Report &&  checkBuilding){
                            ...
                            if(floors1.size()!=0)  {
                                floors{
                                    def badFloor = false
                                    for(x in floors1){
                                        log.debug("For Floor.........."+x)
                                        floor{
                                            id(x.id)
                                            def equips = Equipment.findAllByFloorAndIsDeleted(x,false)
