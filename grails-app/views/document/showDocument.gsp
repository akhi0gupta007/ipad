...
<g:if test="${!isPdf }">
   ...
   <ul class="backUl">
   <li class="fl" ><a href="#" class="syn" onclick="openOverlay('emailOverlay')" id="sendEmail" ><span><g:message code="send.email.message" /></span></a></li>
   <li class="fr paddingRight"><g:link controller="document" action="viewDocument" id="${document.id }" class="back"><span><g:message code="login.customer.back"/></span></g:link></li>
   </ul>
   </g:if>
		<div class="${customerListClass}">
			<div class="commonBox">
				...
				<g:if test="${!isPdf }">
					<div class=" fr marginRight "><a href="#"  id="confirm" class="exportPdf fl userHeading"><font color="#000"><g:message code="document.list.export" /> :</font></a></div>
				<input type="hidden" value="${params.id}" name="templateId" /> <g:message code="document.Document.name"/> :<b>${document.name} </b>
    			<g:link controller="document" action="changeDocument" id="${document.id }"><g:message code="users.user.edit" /></g:link>
    			</g:if>
...
<tr><td>
    		 <g:render template="documentSection" model="[documentItemValueMap:documentItemValueMap,Listing:'details1List',headerList:'details1HeaderList',documentSubSection:'1',fourSideBorderClass:documentItemValueMap.get('details11BorderClass')]"></g:render>
    		</td>
    		</tr>
    		<g:render template="documentSection" model="[documentItemValueMap:documentItemValueMap,Listing:'details1List',headerList:'details1HeaderList',documentSubSection:'2',fourSideBorderClass:documentItemValueMap.get('details12BorderClass')]"></g:render>
...
<tr><td>
 		...
 		<!-- Table and checkpoints g:each code starts here -->
    		<g:render template="documentSection" model="[documentItemValueMap:documentItemValueMap,Listing:'details2List',headerList:'details2HeaderList',documentSubSection:'2',fourSideBorderClass:documentItemValueMap.get('details22BorderClass')]"></g:render>
...
<g:if test="${ documentItemValueMap.get('otherList').size()>0}">
		...
		<tr><td>
    		 <g:render template="documentSection" model="[documentItemValueMap:documentItemValueMap,Listing:'otherList',headerList:'otherHeaderList',documentSubSection:'1',fourSideBorderClass:documentItemValueMap.get('other1BorderClass')]"></g:render>
   		</td>
    		</tr>
    		
    		<!-- Table and checkpoints g:each code starts here -->
    		<g:render template="documentSection" model="[documentItemValueMap:documentItemValueMap,Listing:'otherList',headerList:'otherHeaderList',documentSubSection:'2',fourSideBorderClass:documentItemValueMap.get('other2BorderClass')]"></g:render>
...
<tr>
	<td>
    	 <div class=" pdfOuter fourSideBorder marginTopBottom fl">
    		...
    		<!-- Table and checkpoints g:each code starts here -->
        		<g:render template="documentSection" model="[documentItemValueMap:documentItemValueMap,Listing:'remarksList',headerList:'remarksHeaderList',documentSubSection:'2',fourSideBorderClass:documentItemValueMap.get('remarks2BorderClass')]"></g:render>
...
<g:if test="${documentItemValueMap.get('body1BorderClass') != '' }">
	<tr>
		<td>
    		...
    		<!-- Table and checkpoints g:each code starts here -->
    		  <g:render template="documentSection" model="[documentItemValueMap:documentItemValueMap,Listing:'bodyList',headerList:'bodyHeaderList',documentSubSection:'2',fourSideBorderClass:documentItemValueMap.get('body2BorderClass')]"></g:render>
     ...
     <!-- Body Element display ends here -->
      ...
      <!-- Footer Element display starts here -->
      	<g:if test="${documentItemValueMap.get("footList").size()>0}">
        	<g:render template="documentSection" model="[documentItemValueMap:documentItemValueMap,Listing:'footList',headerList:'footHeaderList',documentSubSection:'2',fourSideBorderClass:'']"></g:render>
        </g:if>
 ...
 <!-- email overlay star from here-->
 <g:if test="${!isPdf }">
<div class="overlay" id="emailOverlay">
<!--overlay Start Here-->
   <div class="overlayMain">
 	<div class="fourSideBorder overlayTopMargin fl">
      <div class="formHeadingBg fullWidth marginBottom">
      	<h2 class="marginLeft"><g:message code="send.email.message"/><a href="#" onclick="closeOverlay('emailOverlay')" class="closeButton activeEditBtn" id="closeEmail">Close</a></h2>
      		<g:formRemote name="groupMail" url="[controller:'site',action:'sendGroupEmail']" onkeypress="return checkEnter(event)" beforeSend="checkmailInput()" before="if(checkmailInput()){" after="}" onComplete="closeOverlay('emailOverlay');">
        		<label class="overlayInputHeading fl "><g:message code="new.customer.email" /></label>
        		<textarea name="recepients" id="suggest3" class="textareaWidth" rows="2" placeholder="<g:message code="email.group.recipients" />"  onkeydown="Javascript: if (event.keyCode==13 || event.keyCode==188 || event.keyCode==186 || event.keyCode==32) checkEmailValid(event,this.value);" onblur="checkEmailValid(event,this.value);"></textarea>
        		<div id="recepientsMessage"  ></div>
        		<label class="overlayInputHeading fl "><g:message code="document.email.subject" /></label>
        		<input type="text" class="textareaWidth" disabled="disabled" value="Document Name: ${document.name }, Document Number: ${document.documentNumber }"/>
        		<input type="hidden" class="textareaWidth" value="Document Name: ${document.name }, Document Number: ${document.documentNumber }" name="mailSubject"/>
        		
      			<label class="overlayInputHeading fl "><g:message code="document.email.message" /></label>
          		<textarea name="message" id="message" class="textareaWidth" rows="3" onblur="removeErrorClass('message','mailMessage');"></textarea>
          		<div id="mailMessage" ></div>
          		<div class="widthHalf marginRight fr">
           			 <div class="cancelCommonBg">
        				<input type="button" class="inputButton fl" onclick="closeOverlay('emailOverlay')" value="<g:message code='users.cancel'/>" />
       				 </div>
...
<div class="overlay" id="confirmOverlay">
	<div class="ovelayPostion">
   		<div class="overlayWidth formHeadingBg overlayTopMargin">
     		<h2 class="marginLeft"><g:message code="document.showdocument.pdf.header" /><a href="#" class="closeButton activeEditBtn" onclick="closeOverlay('confirmOverlay')">Close</a></h2>
      				
      				    <div class="formInput">
							<h4 class="textCenter"><g:message code="document.showdocument.pdf" /></h4>
						</div>
				<div class="formInput  textCenter">
				<input  type="button" class="marginRight" onclick="confirmImagePdf(true);" value="Yes">
				<input type="button" class="marginRight" onclick="confirmImagePdf(false);" value="No">
				<input type="button" onclick="closeOverlay('confirmOverlay');" value="Cancel">
 ...
 <!--overlay end Here-->
	...
	<script src="${resource(dir:'js',file:'groupData.js') }"></script>
